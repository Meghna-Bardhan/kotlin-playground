...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val emptyCellsInBoard = board.getAllCells().filter { cell -> board[cell] == null }
        if (emptyCellsInBoard.isEmpty()) return null
        val randomValue = generateRandomStartValue()
        val randomCellToFill = (emptyCellsInBoard.indices).shuffled().first()
        return emptyCellsInBoard[randomCellToFill] to randomValue
    }
}